...
if (count($topic_ids)) {
	...
	foreach ($items as $k => $item) {
        if ($item['node_reftype'] == 'forums.topics' && isset($topic_rows[$item['node_refid']])) {
            $items[$k]['data'] = $topic_rows[$item['node_refid']];
        }
    }
}
$post_ids = array();
...
if (count($post_ids)) {
	...
	foreach ($items as $k => $item) {
        if ($item['node_reftype'] == 'forums.posts' && isset($post_rows[$item['node_refid']])) {
            $items[$k]['data'] = $post_rows[$item['node_refid']];
        }
    }
}
